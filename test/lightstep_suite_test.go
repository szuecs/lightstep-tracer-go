...
var _ = BeforeSuite(func() {
	// Prevents extraneous error messages that may occur during test teardown because,
	// e.g., the test server is shut down before the tracer's event loop has stopped
	lightstep.SetGlobalEventHandler(func(lightstep.Event) {})
})

var (
	validSpanContext = lightstep.SpanContext{
		TraceID: 12345,
		SpanID:  98765,
...
func testTracer(dependencies func() (testSatellite, lightstep.Options)) {
	var (
	...
	testReporting := func(reportFn func()) {
		...
		It("does not send any spans if the tracer is disabled", func() {
			subject.Disable()
	...
	BeforeEach(func() {
		satellite, options = dependencies()

		options.AccessToken = "test access token"

		subject = lightstep.NewTracer(options)
	})

	It("provides a non-nil Tracer", func() {
		Expect(subject).NotTo(BeNil())
	})

	It("complies with the OpenTracing Tracer interface", func() {
		// Forces a compile-time check
		_, ok := subject.(opentracing.Tracer)
		Expect(ok).To(BeTrue())
	})

	Context("when the max reporting period is reached", func() {
		var (
			maxReportingPeriod = time.Nanosecond * 2
		)

		BeforeEach(func() {
			options.ReportingPeriod = maxReportingPeriod
			options.MinReportingPeriod = time.Nanosecond // otherwise, the flush loop won't get triggered

			subject = lightstep.NewTracer(options)
		})

		testReporting(func() { time.Sleep(maxReportingPeriod) })
	})

	Context("when there are sufficient buffered spans", func() {
		var (
			maxSpans = 4
		)

		BeforeEach(func() {
			options.MaxBufferedSpans = maxSpans
			options.MinReportingPeriod = time.Nanosecond
		...
		testReporting(func() {
			for i := 0; i < maxSpans-1; i++ {
				span := subject.StartSpan(strconv.Itoa(i))
				span.Finish()
			}
		})
	})
	...
	Describe("#Inject", func() {
		Context("OpenTracing Binary format", func() {
			It("successfully injects a valid SpanContext into a io.Writer", func() {
	...
	Describe("#Flush", func() {
		testReporting(func() { subject.Flush(context.Background()) })
	})
}
